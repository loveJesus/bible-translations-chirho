...
-- EXODUS c5-v1 - HIN
-- aur–bād āye Mūsā aur–Hārūn aur–kahā se Firʿaun yū̃ kahā Yahve–ne Parmeshvar Isrāel–kā bhej ko log–apne aur–utsav-manāyẽ mere-liye marusthal–mẽ
...
INSERT INTO gloss (phrase_id, gloss, state, updated_at, source)
SELECT p.id, 'और–उतसव-मनायें', 'UNAPPROVED', NOW(), 'IMPORT'
FROM phrase p JOIN phrase_word pw ON pw.phrase_id = p.id
WHERE pw.word_id = '0200500116' AND p.language_id = (SELECT id FROM language WHERE code = 'hin') AND p.deleted_at IS NULL
ON CONFLICT (phrase_id) DO UPDATE SET gloss = EXCLUDED.gloss, updated_at = EXCLUDED.updated_at, source = EXCLUDED.source;
