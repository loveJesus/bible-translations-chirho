...
-- PROVERBS c3-v10 - SWA
-- ndipo–ghala–zako ghala–zako zitajaa–tele na–divai–mpya mashinikizo–yako yatafurika 

BEGIN;
-- 2000301001: וְיִמָּלְא֣וּ (H4390) → "ndipo–ghala–zako" [opus-4.5-chirho]
WITH np AS (
...
SELECT p.id, 'ndipo–ghala–zako', 'UNAPPROVED', NOW(), 'IMPORT'
...
WHERE pw.word_id = '2000301002' AND p.language_id = (SELECT id FROM language WHERE code = 'swa') AND p.deleted_at IS NULL
ON CONFLICT (phrase_id) DO UPDATE SET gloss = EXCLUDED.gloss, updated_at = EXCLUDED.updated_at, source = EXCLUDED.source;
-- 2000301003: שָׂבָ֑ע (H7647) → "zitajaa–tele" [opus-4.5-chirho]
WITH np AS (
  INSERT INTO phrase (language_id, created_at)
  SELECT (SELECT id FROM language WHERE code = 'swa'), NOW()
  WHERE NOT EXISTS (
...
INSERT INTO gloss (phrase_id, gloss, state, updated_at, source)
SELECT p.id, 'zitajaa–tele', 'UNAPPROVED', NOW(), 'IMPORT'
FROM phrase p JOIN phrase_word pw ON pw.phrase_id = p.id
WHERE pw.word_id = '2000301003' AND p.language_id = (SELECT id FROM language WHERE code = 'swa') AND p.deleted_at IS NULL
ON CONFLICT (phrase_id) DO UPDATE SET gloss = EXCLUDED.gloss, updated_at = EXCLUDED.updated_at, source = EXCLUDED.source;
...
WHERE pw.word_id = '2000301005' AND p.language_id = (SELECT id FROM language WHERE code = 'swa') AND p.deleted_at IS NULL
ON CONFLICT (phrase_id) DO UPDATE SET gloss = EXCLUDED.gloss, updated_at = EXCLUDED.updated_at, source = EXCLUDED.source;
-- 2000301006: יִפְרֹֽצוּ׃ (H6555) → "yatafurika" [opus-4.5-chirho]
WITH np AS (
  INSERT INTO phrase (language_id, created_at)
  SELECT (SELECT id FROM language WHERE code = 'swa'), NOW()
  WHERE NOT EXISTS (
