...
-- PROVERBS c2-v2 - SWA
-- ku–sikiliza kwa–hekima sikio–lako u–elekeze moyo–wako kwa–ufahamu

BEGIN;
-- 2000200201: לְהַקְשִׁ֣יב (H7181) → "ku–sikiliza" [opus-4.5-chirho]
WITH np AS (
...
SELECT p.id, 'ku–sikiliza', 'UNAPPROVED', NOW(), 'IMPORT'
...
WHERE pw.word_id = '2000200203' AND p.language_id = (SELECT id FROM language WHERE code = 'swa') AND p.deleted_at IS NULL
ON CONFLICT (phrase_id) DO UPDATE SET gloss = EXCLUDED.gloss, updated_at = EXCLUDED.updated_at, source = EXCLUDED.source;
-- 2000200204: תַּטֶּ֥ה (H5186) → "u–elekeze" [opus-4.5-chirho]
WITH np AS (
  INSERT INTO phrase (language_id, created_at)
  SELECT (SELECT id FROM language WHERE code = 'swa'), NOW()
  WHERE NOT EXISTS (
...
INSERT INTO gloss (phrase_id, gloss, state, updated_at, source)
SELECT p.id, 'u–elekeze', 'UNAPPROVED', NOW(), 'IMPORT'
FROM phrase p JOIN phrase_word pw ON pw.phrase_id = p.id
WHERE pw.word_id = '2000200204' AND p.language_id = (SELECT id FROM language WHERE code = 'swa') AND p.deleted_at IS NULL
ON CONFLICT (phrase_id) DO UPDATE SET gloss = EXCLUDED.gloss, updated_at = EXCLUDED.updated_at, source = EXCLUDED.source;
