...
-- PROVERBS c1-v9 - SWA
-- kwa–sababu taji la–neema hayo kwa–kichwa–chako na–mikufu kwa–shingo–yako

BEGIN;
-- 2000100901: כִּ֤י ׀ (H3588a) → "kwa–sababu" [opus-4.5-chirho]
WITH np AS (
...
WHERE pw.word_id = '2000100903' AND p.language_id = (SELECT id FROM language WHERE code = 'swa') AND p.deleted_at IS NULL
ON CONFLICT (phrase_id) DO UPDATE SET gloss = EXCLUDED.gloss, updated_at = EXCLUDED.updated_at, source = EXCLUDED.source;
-- 2000100904: הֵ֬ם (H1992) → "hayo" [opus-4.5-chirho]
WITH np AS (
  INSERT INTO phrase (language_id, created_at)
  SELECT (SELECT id FROM language WHERE code = 'swa'), NOW()
  WHERE NOT EXISTS (
...
WHERE pw.word_id = '2000100905' AND p.language_id = (SELECT id FROM language WHERE code = 'swa') AND p.deleted_at IS NULL
ON CONFLICT (phrase_id) DO UPDATE SET gloss = EXCLUDED.gloss, updated_at = EXCLUDED.updated_at, source = EXCLUDED.source;
-- 2000100906: וַ֝עֲנָקִ֗ים (H6060b) → "na–mikufu" [opus-4.5-chirho]
WITH np AS (
  INSERT INTO phrase (language_id, created_at)
  SELECT (SELECT id FROM language WHERE code = 'swa'), NOW()
  WHERE NOT EXISTS (
