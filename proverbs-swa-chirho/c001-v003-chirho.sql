...
-- PROVERBS c1-v3 - SWA
-- kupokea mafundisho ya–busara haki na–hukumu na–unyofu

BEGIN;
-- 2000100301: לָ֭קַחַת (H3947) → "kupokea" [opus-4.5-chirho]
WITH np AS (
...
WHERE pw.word_id = '2000100305' AND p.language_id = (SELECT id FROM language WHERE code = 'swa') AND p.deleted_at IS NULL
ON CONFLICT (phrase_id) DO UPDATE SET gloss = EXCLUDED.gloss, updated_at = EXCLUDED.updated_at, source = EXCLUDED.source;
-- 2000100306: וּמֵישָׁרִֽים׃ (H4339) → "na–unyofu" [opus-4.5-chirho]
WITH np AS (
  INSERT INTO phrase (language_id, created_at)
  SELECT (SELECT id FROM language WHERE code = 'swa'), NOW()
  WHERE NOT EXISTS (
