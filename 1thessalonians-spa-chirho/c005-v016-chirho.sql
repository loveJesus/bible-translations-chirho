...
WITH np AS (
  ...
  WHERE NOT EXISTS (
    ...
    WHERE pw.word_id = '5200501601' AND p.language_id = (SELECT id FROM language WHERE code = 'spa') AND p.deleted_at IS NULL
  )
  RETURNING id
)
...
-- 5200501602: χαίρετε. (G5463) → "gozaos" [opus-4.5-chirho]
WITH np AS (
  INSERT INTO phrase (language_id, created_at)
  SELECT (SELECT id FROM language WHERE code = 'spa'), NOW()
  WHERE NOT EXISTS (
