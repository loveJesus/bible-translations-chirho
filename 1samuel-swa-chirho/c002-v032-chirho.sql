...
-- 1SAMUEL c2-v32 - SWA
-- na–utatazama adui atafanya–vizuri

BEGIN;
-- 0900203201: וְהִבַּטְתָּ֙ (H5027) → "na–utatazama" [opus-4.5-chirho]
WITH np AS (
...
SELECT p.id, 'na–utatazama', 'UNAPPROVED', NOW(), 'IMPORT'
...
WHERE pw.word_id = '0900203202' AND p.language_id = (SELECT id FROM language WHERE code = 'swa') AND p.deleted_at IS NULL
ON CONFLICT (phrase_id) DO UPDATE SET gloss = EXCLUDED.gloss, updated_at = EXCLUDED.updated_at, source = EXCLUDED.source;
-- 0900203206: יֵיטִ֖יב (H3190) → "atafanya–vizuri" [opus-4.5-chirho]
WITH np AS (
  INSERT INTO phrase (language_id, created_at)
  SELECT (SELECT id FROM language WHERE code = 'swa'), NOW()
  WHERE NOT EXISTS (
...
INSERT INTO gloss (phrase_id, gloss, state, updated_at, source)
SELECT p.id, 'atafanya–vizuri', 'UNAPPROVED', NOW(), 'IMPORT'
FROM phrase p JOIN phrase_word pw ON pw.phrase_id = p.id
WHERE pw.word_id = '0900203206' AND p.language_id = (SELECT id FROM language WHERE code = 'swa') AND p.deleted_at IS NULL
ON CONFLICT (phrase_id) DO UPDATE SET gloss = EXCLUDED.gloss, updated_at = EXCLUDED.updated_at, source = EXCLUDED.source;
COMMIT;
