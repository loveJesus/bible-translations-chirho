...
-- 1SAMUEL c5-v8 - SWA
-- tutafanya–nini kwa–sanduku

BEGIN;
-- 0900500810: נַּעֲשֶׂ֗ה (H6213a) → "tutafanya–nini" [opus-4.5-chirho]
WITH np AS (
...
SELECT p.id, 'tutafanya–nini', 'UNAPPROVED', NOW(), 'IMPORT'
FROM phrase p JOIN phrase_word pw ON pw.phrase_id = p.id
WHERE pw.word_id = '0900500810' AND p.language_id = (SELECT id FROM language WHERE code = 'swa') AND p.deleted_at IS NULL
ON CONFLICT (phrase_id) DO UPDATE SET gloss = EXCLUDED.gloss, updated_at = EXCLUDED.updated_at, source = EXCLUDED.source;
-- 0900500811: לַֽאֲרוֹן֙ (H0727) → "kwa–sanduku" [opus-4.5-chirho]
WITH np AS (
  INSERT INTO phrase (language_id, created_at)
  SELECT (SELECT id FROM language WHERE code = 'swa'), NOW()
  WHERE NOT EXISTS (
