...
-- ROMANS c1-v29 - SWA
-- waliojazwa kwa–kila udhalimu uovu tamaa ubaya wamejawa wivu mauaji ugomvi udanganyifu uovu wenye–kunong'ona

BEGIN;
-- 4500102901: πεπληρωμένους (G4137) → "waliojazwa" [opus-4.5-chirho]
WITH np AS (
...
SELECT p.id, 'waliojazwa', 'UNAPPROVED', NOW(), 'IMPORT'
...
WHERE pw.word_id = '4500102904' AND p.language_id = (SELECT id FROM language WHERE code = 'swa') AND p.deleted_at IS NULL
ON CONFLICT (phrase_id) DO UPDATE SET gloss = EXCLUDED.gloss, updated_at = EXCLUDED.updated_at, source = EXCLUDED.source;
-- 4500102905: πλεονεξίᾳ, (G4124) → "tamaa" [opus-4.5-chirho]
WITH np AS (
  INSERT INTO phrase (language_id, created_at)
  SELECT (SELECT id FROM language WHERE code = 'swa'), NOW()
  WHERE NOT EXISTS (
...
WHERE pw.word_id = '4500102906' AND p.language_id = (SELECT id FROM language WHERE code = 'swa') AND p.deleted_at IS NULL
ON CONFLICT (phrase_id) DO UPDATE SET gloss = EXCLUDED.gloss, updated_at = EXCLUDED.updated_at, source = EXCLUDED.source;
-- 4500102907: μεστοὺς (G3324) → "wamejawa" [opus-4.5-chirho]
WITH np AS (
  INSERT INTO phrase (language_id, created_at)
  SELECT (SELECT id FROM language WHERE code = 'swa'), NOW()
  WHERE NOT EXISTS (
