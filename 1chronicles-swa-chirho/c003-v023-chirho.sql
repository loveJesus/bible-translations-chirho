...
-- Na–mwana–wa– Nearia Elioenai na–Hezekia na–Azrikamu watatu

BEGIN;
-- 1300302301: וּבֶן־ (H1121a) → "Na–mwana–wa–" [opus-4.5-chirho]
WITH np AS (
...
WHERE pw.word_id = '1300302302' AND p.language_id = (SELECT id FROM language WHERE code = 'swa') AND p.deleted_at IS NULL
ON CONFLICT (phrase_id) DO UPDATE SET gloss = EXCLUDED.gloss, updated_at = EXCLUDED.updated_at, source = EXCLUDED.source;
-- 1300302303: אֶלְיוֹעֵינַ֧י (H0454b) → "Elioenai" [opus-4.5-chirho]
WITH np AS (
  INSERT INTO phrase (language_id, created_at)
  SELECT (SELECT id FROM language WHERE code = 'swa'), NOW()
  WHERE NOT EXISTS (
